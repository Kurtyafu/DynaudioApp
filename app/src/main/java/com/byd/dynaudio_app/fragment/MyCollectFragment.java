...
import com.byd.dynaudio_app.databinding.LayoutFragmentMyCollectBinding;
...
public class MyCollectFragment extends BaseFragment<LayoutFragmentMyCollectBinding, BaseViewModel> {
    ...
    protected void initListener() {
        ...
        mDataBinding.txtPlayAll.setOnClickListener(view -> {/*顺序播放*/
            if (mDataBinding.tabLayout.firstItemSelected()) {
                ...
                MusicPlayManager.getInstance().playMusic(mMusicList, bean, 0, "collect");
            } else {
                //有声节目播放
                MusicListBean bean = mAudioList.get(0);
                MusicPlayManager.getInstance().playMusic(mAudioList, bean, 0, "collect");
            }
        });
    ...
    protected int getLayoutId() {
        return R.layout.layout_fragment_my_collect;
    }
    ...
    private void selectDataSource() {
        if (UserController.getInstance().isLoginStates()) {
            ...
            List<MusicListBean> musicList = DBController.queryMusicCollect(mContext);
            List<MusicListBean> audioList = DBController.queryAudioCollect(mContext);
    ...
    private void requestMusicData() {
        ...
        ApiClient.getInstance().getMusicCollect(userId).subscribe(new BaseObserver<>() {
            @Override
            protected void onSuccess(BaseBean<List<MusicListBean>> bean) {
                if (bean.getData() != null) {
                    List<MusicListBean> list = bean.getData();
                    // 收藏接口返回的数据竟然是false你敢信? Family, who knows?
                    for (MusicListBean listBean : list) {
                        listBean.setCollectFlag(true);
                    }
    ...
    private void requestAudioData() {
        String userId = UserController.getInstance().getUserId();
        ApiClient.getInstance().getAudioCollect(userId).subscribe(new BaseObserver<>() {
            @Override
            protected void onSuccess(BaseBean<List<MusicListBean>> bean) {
                if (bean.getData() != null) {
                    List<MusicListBean> list = bean.getData();
    ...
    private void initMusicListClickListener() {
        mMusicAdapter.setClickListener(new IItemOnClickListener() {
            ...
            public void onClick(int position) {
                ...
                MusicPlayManager.getInstance().playMusic(mMusicList, bean, position, "collect");
            }
    ...
    private void initAudioListClickListener() {
        mAudioAdapter.setClickListener(new IItemOnClickListener() {
            ...
            public void onClick(int position) {
                ...
                MusicPlayManager.getInstance().playMusic(mAudioList, bean, position, "collect");
            }
    ...
    private void deletePlayRecord() {
        ...
        dialog.setClickListener(new IOnClickListener() {
            ...
            public void onPositiveButtonListener() {
                if (mDataBinding.tabLayout.firstItemSelected()) {// 音乐
                    for (MusicListBean bean : mMusicList) {
                        if (bean.isSelected()) {
                            bean.setCollectFlag(false);
                            CollectController.getInstance().collect(false, bean);
                ...
                deleteComplete();
                CustomToast.makeText(mContext, "成功删除" + mDelNumber + "条收藏内容", Toast.LENGTH_SHORT).show();
            }
        });
        dialog.show();
    }
    ...
    private void deleteSingleMusicItem(int position) {
        ...
        MusicPlayManager.getInstance().setCollect(mMusicList.get(position), false);
        deleteSelectedMusicItem();
        deleteComplete();
    }
    ...
    private void deleteSingleAudioItem(int position) {
        ...
        MusicPlayManager.getInstance().setCollect(mAudioList.get(position), false);
        deleteSelectedAudioItem();
        deleteComplete();
    }
