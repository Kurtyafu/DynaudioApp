package com.byd.dynaudio_app.custom.recycler;
...
/**
 * 带回弹效果的RecyclerView。 横向
 */
public class OverScrollHorizontalRecyclerView extends RecyclerView implements View.OnTouchListener {
    ...
    public OverScrollHorizontalRecyclerView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public OverScrollHorizontalRecyclerView(Context context, @Nullable AttributeSet attrs, int defStyle) {
    ...
    private void initAnimationAttributes(View view, AnimationAttributes attributes) {
        attributes.mProperty = View.TRANSLATION_X;
        attributes.mAbsOffset = view.getTranslationX();
        attributes.mMaxOffset = view.getWidth();
    }
    ...
    private void translateViewAndEvent(View view, float offset, MotionEvent event) {
        view.setTranslationX(offset);
        event.offsetLocation(0f, offset - event.getX(0));
    }
